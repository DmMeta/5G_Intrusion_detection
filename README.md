...
## Prometheus and Grafana Monitoring in Kubernetes Environment

Integrating **Prometheus** and **Grafana** into a Kubernetes environment offers a comprehensive solution for monitoring and visualizing the performance of Kubernetes clusters, applications and infrastructure. [Prometheus][Prometheus-link] serves as the monitoring and alerting tool, collecting metrics from Kubernetes resources, while [Grafana][Grafana-link] provides powerful visualization capabilities, enabling users to create customized dashboards for monitoring Kubernetes metrics effectively.

In our one-node Kubernetes cluster, Prometheus was deployed to scrape metrics from the node's pods and services. Prometheus utilizes the Kubernetes service discovery mechanism to automatically locate monitoring targets, such as pods and services, and then employs scrapers to retrieve metrics from them.

Once Prometheus collects the metrics, it provides an HTTP endpoint from which Grafana can retrieve these metrics. Grafana utilizes the Prometheus datasource plugin to connect to Prometheus's HTTP endpoint and retrieve the metrics.

Upon receiving the metrics from Prometheus, Grafana can use this data to dynamically create dashboards that visualize Kubernetes metrics, such as CPU - memory usage and network traffic. This integrated process allows to monitor the health and performance of the Kubernetes infrastructure in real-time.

An illustrative image showcasing the dashboards generated by Grafana at an arbitrary point in time is provided below:

<br>
<p align="center">
 <img src="./images_media/dashboards_grafana.png"  alt="Grafana Dashboards">
    <br>
    <em><i>Grafana Dashboards Snapshot</i></em>
</p>
<br>
